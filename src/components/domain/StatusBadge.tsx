...
export function StatusBadge({ status }: { status: string | null | undefined }) {
  ...
  const meta = COLORS[key] ?? { color: "var(--color-muted)", label: status ?? "—" };
...
export function SourceLabel({ source }: { source: string | null | undefined }) {
  ...
  const config: Record<string, { label: string; icon: string; bg: string; fg: string }> = {
    fracfocus: { label: "FracFocus", icon: "▣", bg: "var(--color-frac)", fg: "white" },
    uic: { label: "SWD Wells", icon: "◆", bg: "var(--color-swd)", fg: "white" },
    h10: { label: "SWD Monitor", icon: "⬡", bg: "var(--color-swd)", fg: "white" },
    texnet: { label: "TexNet", icon: "◎", bg: "var(--color-accent)", fg: "white" },
    usgs: { label: "USGS", icon: "●", bg: "var(--color-status-running)", fg: "white" },
    iris: { label: "IRIS Stations", icon: "▲", bg: "var(--color-station)", fg: "white" },
  };

  const c = config[s] ?? { label: source ?? "—", icon: "?", bg: "var(--color-muted)", fg: "white" };
