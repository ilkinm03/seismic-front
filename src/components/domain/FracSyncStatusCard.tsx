              ...
              <div className="rounded-2xl border border-[var(--color-border)]/60 bg-[var(--color-card-elevated)] p-4 transition-colors hover:border-[var(--color-accent)]/20">
                ...
                <div className="mt-2 flex items-baseline gap-1.5">
                  <span className="text-xl font-black tracking-tight text-[var(--color-fg)]">
                    {data?.last_sync_at
                      ? relativeTime(data.last_sync_at).split(" ")[0]
                      : "—"}
                  </span>
              ...
              >
                <div className="max-h-[240px] overflow-y-auto p-2">
                  {data?.csv_files?.map((f) => (
                    <div
                      key={f.filename}
                      className="group flex items-center justify-between rounded-lg px-3 py-2 text-[11px] transition-colors hover:bg-[var(--color-accent)]/5"
                    >
                      <span
                        className="font-mono truncate text-[var(--color-muted)] group-hover:text-[var(--color-fg)]"
                        title={f.filename}
                      >
                        {f.filename}
                      </span>
              ...
              {data?.etag && (
                <div className="border-t border-[var(--color-border)]/40 bg-[var(--color-card-elevated)]/30 px-4 py-2.5">
                  <div className="flex items-center justify-between gap-4">
                    <span className="shrink-0 text-[9px] font-bold uppercase tracking-widest text-[var(--color-subtle)]">
                      Registry ETag
                    </span>
                    <span className="font-mono text-[9px] text-[var(--color-muted)] truncate select-all">
                      {data.etag}
                    </span>
                  </div>
                </div>
