      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black tracking-tight text-[var(--color-fg)]">
                Synchronization History
              </h2>
              <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)]">
                Audit log of all data pipeline activity
              </p>
            </div>
          </div>
        ...
        <div className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-card)] overflow-hidden shadow-sm">
          {history.isError ? (
            <div className="p-12">
              <ErrorState
                title="Could not fetch synchronization history"
                description={String(history.error)}
              />
            </div>
          ) : history.isLoading ? (
